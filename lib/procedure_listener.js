...
class procedure_listener {
    ...
    constructor(client, publisher, serviceName, queueName, maxConsume, functionLogic) {
        ...
        this.handleListenerEvents(this.client);
    ...
    handleListenerEvents(redisClient) {
        redisClient.on("ready", () => {
            this.startListener();
            console.log("reday signal from procedure listener ->>>>");
        });

        redisClient.on("error", (error) => {
            console.error("error signal from procedure listener ->>>>>>\n", error);
            // let data = JSON.stringify({ error, msg: "Error RPC connection error ", time: new Date() });
        });

        redisClient.on("connect", () => {
            console.log("connect signal from procedure listener ->>>>>>")
    ...
    async processMSG(callerMSG) {
        ...
        let response;
        try {
            response = await this.functionLogic(callerMSG);
        } catch (error) {
            response = {
                result: null,
                status: {
        ...
        response = Object.assign({}, callerMSG, {
            result: response
        });
        response.reqId = callerMSG.header.id;
        await this.publisher.lpush(callerMSG.header.consumerQueue, JSON.stringify(response)); //start rpc
        if (this.currentConsLen == this.maxConsume) this.EventEmitter.emit("open");
        this.currentConsLen--;
        response = callerMSG = undefined;
        return;
    }
}
